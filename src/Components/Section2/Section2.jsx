...
export default function Section2() {
  ...
  return (
    <div className=" global-padding grid grid-cols-1  md:grid-cols-2 gap-6 p-4 sm:p-6 md:p-8 lg:p-12 bg-white shadow-2xl rounded-3xl">
  ...
  {['globalHappyClients1', 'globalHappyClients2', 'globalHappyClients3', 'globalHappyClients'].map((key, index) => (
    <div key={index} className="flex flex-col items-center space-y-6 md:space-y-8">
      <div className="flex items-center space-x-2">
        <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold mb-1 md:mb-2">{index + 1}K</h1>
        <span className="text-xs sm:text-sm md:text-base lg:text-lg">{t(key)}</span>
      </div>
    </div>
  ))}
</div>
